  ...
  for (const ev of events) {
    ...
    if (ev.type === 'tool/call') {
      ...
      try {
        ...
        hit = candidates.some((p) => (/\brun\/[^\s"']*\.(md|json|txt|log|csv|yaml|yml|dat)\b/i.test(p) || /\.md\b/i.test(p)))
      } catch { /* fall through */ }
      if (!hit) {
...
let pass = 0
let fail = 0
const lines = ['# reviewer 触发判据单元测试结果', '', `- 日期：${new Date().toISOString().slice(0, 10)}`, '- 命令：`node tests/reviewer-deliverable.mjs`', '- 覆盖：L0-1 / L1-1..3 / L2-4 / L2-5 修复后的 hasDeliverable 等价逻辑', '', '| 用例 | 期望 | 实测 | 关联 |', '|---|---|---|---|']
