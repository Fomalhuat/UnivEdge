...
export function apply(ctx: Context): void {
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        script_path: { type: 'string', description: '要提交的 SLURM 脚本路径（绝对路径）' },
        h: { type: 'string', description: '步长 h（如 0.005）' },
        lmax: { type: 'string', description: '截断 lmax（如 39）' },
        rp: { type: 'string', description: '轨道半径 r_p（如 10）' },
        ref: { type: 'string', description: '对照值/参照来源（如 PRD77 Table I ∂t(10M)=1.747254e-5）' },
