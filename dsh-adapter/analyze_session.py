...
指标口径（与 UnivEdge 加载协议 L0-L2 + VERIFICATION 检查项对齐）：
  ...
  - L2 契约：assistant 输出或写入文件含"任务契约/完成定义/验证标准/测试计划"
  - L2 自省：assistant 输出含"启动自省/根因/B1/B2/B3/B4"（B1-B4+P 逐条内省）
  - 工具代算：tool/call 含 bash/python/sympy/mathematica 等执行工具
  - 锚点对照：web_search 或引用 DLMF/Wikipedia/文献/独立参照
"""
...
def extract(events: list[dict]) -> dict:
    ...
    # 指标判定。
    # 口径：L0/L1 看注入侧（user 文本）；L2 契约/自省、锚点看 assistant 输出（agent 的实际行为，
    # 避免被 AGENTS.md 自动注入的摘要文字污染——它本身含"任务契约/完成定义"等词）。
    metrics = {
        ...
        'L2 契约(产出)': bool(re.search(r'任务契约|contract\.md|完成定义', all_ast)),
        'L2 自省(逐条)': bool(re.search(r'启动自省|基层已加载|逐条内省|B1.*B2.*B3.*B4', all_ast, re.S)),
        '工具代算': any(n in ('bash', 'python', 'exec', 'code', 'run') or 'sympy' in all_ast.lower()
                        for n in tool_names),
        '锚点对照': bool(re.search(r'web_search|DLMF|Wikipedia|MathWorld|文献|参照值|基准', all_ast)),
    }
    return {
